...
export default function LobbyPage() {
  ...
  async function startMatch() {
    ...
    try {
      ...
      if (response.status === 403) {
        ...
        alert(
          data.error ?? "The match is already in progress or cannot be started right now."
        );
      } else if (!response.ok) {
  ...
  async function kickPlane(planeId: string) {
    ...
    try {
      ...
      if (response.status === 403) {
        alert("You are not authorized to kick planes from here. Try again in the app.");
      } else if (!response.ok) {
  ...
  const renderPlaneCard = (
    ...
    return (
      <div
        key={plane.planeId}
        className="w-full rounded-2xl border border-skyblue/30 bg-gradient-to-r from-darkernavy/90 via-darkernavy/70 to-skyblue/25 px-6 py-5 text-lg flex items-center gap-5 shadow-md shadow-navy/60 min-h-[110px] transition-all duration-200 hover:border-skyblue/50 hover:shadow-lg"
      >
        ...
        {variant === "joined" && (
          <button
            ...
            disabled={isKicking}
            className={`ml-2 px-4 py-2.5 text-sm font-semibold rounded-xl border transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-red-500/50 ${
              isKicking
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{
          background: 'linear-gradient(-180deg, #110f44, #000000, #000000, #110f44)'
        }}
      ></div>
      
      <div className={`flex flex-col justify-center items-center flex-1 max-w-[1920px] mx-auto w-full py-6 my-auto relative z-10 transition-opacity duration-500 ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}>
      ...
      <div className="flex-1 flex flex-col lg:grid lg:grid-cols-[minmax(0,1.6fr)_minmax(260px,0.8fr)_minmax(0,1.6fr)] gap-6 items-stretch relative z-10">
        ...
        <div className="flex flex-col items-center justify-center gap-6 w-full order-1 lg:order-2">
          ...
          <div 
            ...
            style={{ 
              boxShadow: '0 0 20px rgba(153,207,255,0.6), 0 0 40px rgba(153,207,255,0.3)',
          ...
          >
            <Image
              ...
              style={{ filter: 'drop-shadow(0 0 15px rgba(173,0,0,0.6)) drop-shadow(0 0 30px rgba(173,0,0,0.4))' }}
            />
          </button>
        </div>
          ...
          <div className="flex-1 overflow-y-auto pr-1 space-y-3">
            {joinedPlanes.length === 0 && (
              <div className="text-center mt-8 py-8">
                <p className="text-skyblue/70 text-sm font-medium">Waiting for players to join</p>
                <p className="text-skyblue/50 text-xs mt-2">Players will appear here when they join</p>
              </div>
            )}
